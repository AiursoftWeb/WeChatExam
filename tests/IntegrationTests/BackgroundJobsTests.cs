...
public class BackgroundJobsTests
{
    ...
    public async Task JobQueueBasicOperationsTest()
    {
        ...
        var queue = _server!.Services.GetRequiredService<ServiceTaskQueue>();
    ...
    public async Task CreateTestJobViaUITest()
    {
        ...
        // Step 3: 通过 Trigger 端点触发 DummyJob
        var triggerContent = new FormUrlEncodedContent(new Dictionary<string, string>
        {
            { "jobTypeName", "DummyJob" },
            { "__RequestVerificationToken", token }
        });
        var triggerResponse = await _http.PostAsync("/Jobs/Trigger", triggerContent);
    ...
    public async Task CreateBothJobsViaUIAndVerifyParallelExecutionTest()
    {
        ...
        // Step 2: 获取CSRF令牌并触发 DummyJob
        var tokenA = await GetAntiCsrfToken("/Jobs");
        var triggerDummyContent = new FormUrlEncodedContent(new Dictionary<string, string>
        {
            { "jobTypeName", "DummyJob" },
            { "__RequestVerificationToken", tokenA }
        });
        await _http.PostAsync("/Jobs/Trigger", triggerDummyContent);

        // Step 3: 获取CSRF令牌并触发 OrphanAvatarCleanupJob
        var tokenB = await GetAntiCsrfToken("/Jobs");
        var triggerOrphanContent = new FormUrlEncodedContent(new Dictionary<string, string>
        {
            { "jobTypeName", "OrphanAvatarCleanupJob" },
            { "__RequestVerificationToken", tokenB }
        });
        await _http.PostAsync("/Jobs/Trigger", triggerOrphanContent);
